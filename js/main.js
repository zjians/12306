$(function() {
  ...
  $(document).on('click', '.train', e => {
    ...
    if (!station_names_array.length) {
      ...
      const setCLeftTicketUrl =
        "document.body.setAttribute('data-query-url', CLeftTicketUrl);";
      script.innerHTML =
        "document.body.setAttribute('data-station-name', station_names);" +
        setCLeftTicketUrl;
  ...
  function handleGetDataForTr(el_tr) {
    ...
    el_tr.each(function(i) {
      ...
      if (isEnabled) {
        ...
        $(this).append(
          `<td class='zj-loading'><img src='${chrome.extension.getURL(
  ...
  function handleSuccessCallback(currentStationIndex, el_tr, res) {
    if (!res.data || !res.data.result.length) {
      $(el_tr[currentStationIndex])
        .children('.zj-loading')
        .remove();
      $(el_tr[currentStationIndex]).append(
        `<td><span class='zj-disabled'>--</span></td>`
      );
      return;
    }
        ...
        if (station_query.station_train_code === station_train_code) {
          if (station_query.canWebBuy === 'Y') {
            var html = `<td style='width: 210px;'>
                          <a href="javascript:" onclick="checkG1234('${
                            item.secretStr
                          }', '${station_query.start_time}', '${
              station_query.train_no
            }', '${station_query.from_station_telecode}', '${
              station_query.to_station_telecode
            }')">
                            购买 <i class='info-wrapper'>(二等:${
                              station_query.ze_num
                            } 一等: ${station_query.zy_num} 商务: ${
              station_query.swz_num
            })</i>
                          </a>
                        </td>`;
            $(el_tr[currentStationIndex]).append(html);
            $('#train_div_').width('500px');
            $('#train_span_').width('500px');
            $('.station .station-bd table td').css('width', '55px');
          } else {
            $(el_tr[currentStationIndex]).append(
              `<td><span class='zj-disabled'>购买</span></td>`
            );
          }
          return true;
        } else if (index === stationData.length - 1) {
          $(el_tr[currentStationIndex]).append(
            `<td><span class='zj-disabled'>重试</span></td>`
          );
        }
      });
    }
